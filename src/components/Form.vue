  ...
  export default defineComponent({
    ...
    methods: {
      async createProduct(product: CreateProduct) {
        const body = product;
        try {
          const response = await api.post("3000/", body);
  ...
  <section class="cadastro">
    ...
    <form class="form" @submit="handleCreateProduct($event)">
      <div class="form__field">
        ...
        <input type="text" name="titulo" id="titulo" v-model="product.titulo">
      </div>
      <div class="form__field">
    ...
    <p class="feedback" :class="[isSuccess ? 'success' : 'error']" v-show="isViewFeedback">
      {{
        isSuccess
